...
get_employee_roster_from_csv <- function(employee_csv){
  ...
  # Locate required columns (per your screenshot: "Time In", "Time out", "Shift", "Employee ID")
  ...
  if (!length(b0)) warning("No employees with Time In/Out mapped to 0–8 (M). Will fall back to any available employee if needed.")
  if (!length(b1)) warning("No employees with Time In/Out mapped to 8–16 (D). Will fall back to any available employee if needed.")
  if (!length(b2)) warning("No employees with Time In/Out mapped to 16–24 (N). Will fall back to any available employee if needed.")
