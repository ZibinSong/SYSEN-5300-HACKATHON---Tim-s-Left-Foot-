# SigmaView — Dashboard + Safe Loader (Threshold-violations via *Problems() scorers)
# -------------------------------------------------------------------
...
# ------------ Group-level problem scorers (violations only) ------------
TimeProblems <- function(Data, NormThreshold, RawThreshold, globalAvg, globalNorm){
  Data %>%
    mutate(Group = sprintf("%02d:00", as.integer(timeIN_hr))) %>%
    group_by(Group) %>%
    summarise(
      n = n(),
      AvgDeviation = mean(interval_deviation, na.rm = TRUE),
      NormAvg      = mean(normalized_intervalDiff, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(
      DeltaRaw  = AvgDeviation - globalAvg,
      DeltaNorm = NormAvg - globalNorm,
      Raw_Threshold_Violation  = DeltaRaw  > RawThreshold,
      Norm_Threshold_Violation = DeltaNorm > NormThreshold
    ) %>%
    filter(Raw_Threshold_Violation | Norm_Threshold_Violation) %>%
    arrange(desc(pmax(tidyr::replace_na(DeltaRaw,0),0) + pmax(tidyr::replace_na(DeltaNorm,0),0))) %>%
    mutate(Type = "Time") %>%
    select(Type, Group, n, AvgDeviation, NormAvg, DeltaRaw, DeltaNorm,
           Raw_Threshold_Violation, Norm_Threshold_Violation)
}

EmployeeProblems <- function(Data, NormThreshold, RawThreshold, globalAvg, globalNorm){
  Data %>%
    mutate(Group = as.character(Employee_ID)) %>%
    group_by(Group) %>%
    summarise(
      n = n(),
      AvgDeviation = mean(interval_deviation, na.rm = TRUE),
...
WingProblems <- function(Data, NormThreshold, RawThreshold, globalAvg, globalNorm){
  Data %>%
    mutate(Group = dplyr::coalesce(as.character(Wing), "Unknown")) %>%
    group_by(Group) %>%
    summarise(
      n = n(),
      AvgDeviation = mean(interval_deviation, na.rm = TRUE),
...
SeverityProblems <- function(Data, NormThreshold, RawThreshold, globalAvg, globalNorm){
  Data %>%
    mutate(Group = paste0("Severity ", as.integer(Severity))) %>%
    group_by(Group) %>%
    summarise(
      n = n(),
      AvgDeviation = mean(interval_deviation, na.rm = TRUE),
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    tags$head(tags$style(HTML("
      ...
      #violation_table table.dataTable tbody td, 
      #violation_table table.dataTable thead th { color:#f0f6ff !important; }
    "))),
    ...
    div(style="padding: 10px 15px;",
        selectInput("file_choice", "Hospital System",
                    choices = {
                      opts <- c()
                      if (nrow(dplyr::filter(DATA, source=="MedSize")))  opts <- c(opts, "Med Patient Intake"="MedSize")
                      if (nrow(dplyr::filter(DATA, source=="UCSF")))     opts <- c(opts, "UCSF Patient Intake"="UCSF")
                      if (nrow(dplyr::filter(DATA, source=="Good Sam"))) opts <- c(opts, "Good Sam"="Good Sam")
                      if (length(opts) > 1) c("All Systems"="ALL", opts) else opts
                    },
                    selected = ifelse("ALL" %in% names(
                      {opts <- c()
                      if (nrow(dplyr::filter(DATA, source=="MedSize")))  opts <- c(opts, MedSize="MedSize")
                      if (nrow(dplyr::filter(DATA, source=="UCSF")))     opts <- c(opts, UCSF="UCSF")
                      if (nrow(dplyr::filter(DATA, source=="Good Sam"))) opts <- c(opts, GoodSam="Good Sam"); opts}), "ALL",
                      ifelse(nrow(dplyr::filter(DATA, source=="Good Sam")),"Good Sam",
                             ifelse(nrow(dplyr::filter(DATA, source=="MedSize")),"MedSize","UCSF")))),
        ...
        # Thresholds for violation subset (deltas vs global slice means)
        numericInput("raw_thresh",  "Raw deviation threshold (Δ mins over slice mean)", value = 15, min = 0, step = 1),
        numericInput("norm_thresh", "Normalized diff threshold (Δ over slice mean)",    value = 0.25, min = 0, step = 0.01),
  ...
  dashboardBody(
    ...
    fluidRow(
      ...
      box(title="Subset Violating Thresholds",
          width = 4, solidHeader = TRUE, status = "primary",
          DTOutput("violation_table", width = "100%"))
    ),
    fluidRow(
...
server <- function(input, output, session){
  ...
  # Secondary options — unique FIRST, then names
  ...
  # ---- Threshold-violation subset uses the four *Problems() outputs only ----
  output$violation_table <- renderDT({
    Data <- filtered_visits()
    if (!nrow(Data)) {
      return(datatable(tibble(Note="No data in current slice."), options=list(dom='t'), rownames=FALSE))
    }
    RawThreshold  <- req(input$raw_thresh)
    NormThreshold <- req(input$norm_thresh)
    
    # Baselines computed on current filtered slice (global to the slice)
    GlobalAvg  <- mean(Data$interval_deviation, na.rm = TRUE)
    GlobalNorm <- mean(Data$normalized_intervalDiff, na.rm = TRUE)
    
    TimeScores     <- TimeProblems(Data, NormThreshold, RawThreshold, GlobalAvg, GlobalNorm)
    EmployeeScores <- EmployeeProblems(Data, NormThreshold, RawThreshold, GlobalAvg, GlobalNorm)
    WingScores     <- WingProblems(Data, NormThreshold, RawThreshold, GlobalAvg, GlobalNorm)
    SeverityScores <- SeverityProblems(Data, NormThreshold, RawThreshold, GlobalAvg, GlobalNorm)
    
    Scores <- dplyr::bind_rows(TimeScores, EmployeeScores, WingScores, SeverityScores)
    
    if (!nrow(Scores)) {
      return(datatable(tibble(Note="No groups violate the selected thresholds."),
                       options=list(dom='t'), rownames=FALSE))
    }
    
    datatable(
      Scores,
      options = list(pageLength = 8, dom = 'tip', ordering = TRUE),
      rownames = FALSE,
